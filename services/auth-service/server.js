...
class AuthServer {
  ...
  setupRoutes() {
    ...
    const authRoutes = new AuthRoutes(this.database, this.database.redis);
    this.app.use('/api/auth', authRoutes.getRouter());  // Changed from /auth to /api/auth for better organization
    ...
    // Route 404 - Use regular string instead of wildcard
  ...
  async initializeDatabase() {
    try {
      ...
      // Initialisation des tables avec gestion d'erreur améliorée
      try {
...
📖 API Docs: http://localhost:${this.port}/api-docs
      ...
      this.server.timeout = 30000; // 30 secondes
  ...
  async gracefulShutdown(signal) {
    ...
    try {
      ...
      // Fermer le publisher d'événements
